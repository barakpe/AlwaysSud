---
name: cloud-measure
description: Run the cloud half of one AlwaysSud iteration on the BIU RC cloud - stage hw/ and sw/ into $MY_K5_PROJ, synthesise with qsyn_xlr, simulate easy1/20blanks/51blanks, run the correctness gate, optionally build the bitstream with comp_fpga, and emit the HANDOFF block. Use when asked to measure, baseline, or benchmark a tag/branch of the alwaysud accelerator.
---
...
`sim_<board>.txt` (app stdout - this is the measurement), `sim_<board>.sim.txt`
(simulator side), `cycles.txt`, `HANDOFF.txt`.
...
A correctness mismatch stops the run **before** `comp_fpga`. A faster wrong answer
is not a result.
...
2. **Sourcing the project setup clobbers `"$@"`.** After
   `source startProject.bash`, `$1` is a prompt string. Capture your arguments on
   the first line of the script, before sourcing anything.
...
5. **Two different LE numbers exist.** The headline `Total FPGA Logic Elements`
...
6. **`$QSYN/basic.sdc` does not exist**, so the standalone check runs with **no
...
7. **The two sim processes are peers, not parent/child.** `launch_k5_app` is the
...
8. **Never let two simulators be alive at once.** Because the port is derived from
...
9. **`hard1` is never simulated.** ~30 h of RTL. `sim_board.sh` refuses it.

10. **Staging deletes the synthesis reports.** `hw/xlrs/alwaysud/qsyn_output_files/`
...
11. **The archived reports are gitignored.** `.gitignore` excludes
...
12. **`.f` files: only `//` comments are skipped.** `dotf_to_qsf()` drops lines whose
...
13. **`-hri` is selected by a substring match on the top name**
...
14. **`comp_fpga` moves the `.sof` to the Desktop.** It lands in
...
15. **`prog_fpga` does not exist on the cloud.** Laptop only.
