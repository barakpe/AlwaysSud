...
# A release may name a design directory it was BUILT FROM that is not the one
# promoted into hw/xlrs at its commit - hw/xlrs holds one design at a time, and
# use_design.sh (which such a release REQUIRES you to run) therefore makes the
# tree diverge from that commit on purpose. In that case the plain commit gate
# cannot pass, so check the stronger thing instead: that hw/xlrs/alwaysud is
# byte-identical to the built_from directory AT that commit, and that sw/apps -
# which use_design.sh never touches - is still exactly the commit's.
# With no built_from: line this is the original gate, unchanged.
BUILT_FROM=$(grep -m1 '^built_from:' "$DL/NOTES.txt" | awk '{print $2}' | sed 's:/*$::')
if [ -z "$BUILT_FROM" ]; then
  ...
  echo "  release names built_from: $BUILT_FROM - checking the tree against THAT, not the commit's hw/xlrs"
  git -C "$REPO" diff --quiet "$SHA" -- sw/apps     || { git -C "$REPO" diff --stat "$SHA" -- sw/apps; die "sw/apps drifted from $SHA. use_design.sh never touches sw/apps, so this is real drift."; }
  echo "  PASS  sw/apps matches the bitstream's commit ($SHA)"
  n=0
  for f in "$REPO/hw/xlrs/$XLR"/*; do
    b=$(basename "$f")
    have=$(md5sum "$f" | cut -d' ' -f1)
    want=$(git -C "$REPO" show "$SHA:$BUILT_FROM/$b" 2>/dev/null | md5sum | cut -d' ' -f1)
    [ "$want" = "$have" ] || die "hw/xlrs/$XLR/$b is not $BUILT_FROM/$b at $SHA. Run ./explore/use_design.sh first."
    n=$((n+1))
  done
  # and nothing in built_from is missing from the tree
  for b in $(git -C "$REPO" ls-tree --name-only "$SHA:$BUILT_FROM"); do
    [ -f "$REPO/hw/xlrs/$XLR/$b" ] || die "$BUILT_FROM/$b is missing from hw/xlrs/$XLR."
  done
  echo "  PASS  all $n files in hw/xlrs/$XLR are byte-identical to $BUILT_FROM at $SHA"
fi
...

{
  ...
  if [ -z "$BUILT_FROM" ]; then
    ...
    echo "gate        : built_from check - sw/apps == $SHA, and hw/xlrs/$XLR byte-identical"
    echo "              to $BUILT_FROM at $SHA (the plain commit gate cannot pass here)  ->  clean"
  fi
  echo
